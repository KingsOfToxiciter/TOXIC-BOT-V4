...
module.exports = {
  ...
  onStart: async function ({ message, event, args }) {
    ...
    if (!uids) {
      message.reply("You must reply to a message or tag a person");
      return;
    }


    try {
      const imagePath = await bal(uids);
      await message.reply({
        body: "「 is that true?🥰❤️ 」",
...
async function bal(uids) {
  const avatarone = await jimp.read(`https://graph.facebook.com/${uids}/picture?width=512&height=512&access_token=6628568379%7Cc1e620fa708a1d5696fb991c1bde5662`);
