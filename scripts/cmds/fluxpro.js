module.exports = {
  config: {
    ...
    guide: {
      vi: "",
      en: "{pn} prompt to generate image with flux-pro ai",
    },
  },
  ...
  onStart: async function ({ api, args, message, event }) {
    try {
      ...
      const waitingMessage = await message.reply("✨ | creating your flux-pro request...");
      api.setMessageReaction("⏱️", event.messageID, () => {}, true);
      const startTime = new Date().getTime();

      const h = global.GoatBot.config.api.hasan;
      const API = `${h}/fluxpro?prompt=${encodeURIComponent(prompt)}`;
